...
* [pydotplus](http://pydotplus.readthedocs.io/): Used to visualize the decision trees in the random forests trained by Maat.
...
* The verdicts (i.e., 1 for malicious, 0 for benign, and -1 for unknown), given by the intersection of the most correct **and** stable ```VirusTotal``` scanners based on the scan reports of apps in our __AMD+GPlay__ dataset gathered between November 2018 and September 2019. See **User Manual** for information about how to retrieve the "most correct" and "most stable" scanners. Please beware that the point in time on which you train Maat's labeling strategies **and** the training dataset might impact the list of features you retrieve:  
  * ```AhnLab-V3```
  * ```Avira```
  * ```Babable```
  * ```CAT-QuickHeal```
...
The following list enumerates the selected naive features, viz. the verdicts of ```VirusTotal``` scanners that train the best performing ML-based labeling strategies on our __AMD+GPlay__ training dataset. Please beware that the point in time on which you train Maat's labeling strategies **and** the training dataset might impact the list of features you retrieve:
...
* If no traces of malicious code are found, disassemble the shared object libraries used by (![equation](http://www.sciweavers.org/tex2img.php?eq=\alpha&bc=White&fc=Black&im=jpg&fs=12&ff=arev&edit=)) using [```Gidhra```](https://github.com/NationalSecurityAgency/ghidra) and inspect ```C/C++``` code.
...
The ```SHA1``` hashes, a short description, and the ```VirusTotal``` __positives__:__total__ fields of apps in the **Homegrown** dataset, as of November 8th, 2019.
...
The ```SHA1``` hashes of ten apps randomly sampled from the [AMD](http://amd.arguslab.org/) dataset, the number of scanners deeming them as malicious as of November 8th, 2019, their malware types, and whether ```BitDefender```'s version 3.3.063 and ```Panda```'s version 3.4.5 managed to detect (:heavy_check_mark:) them.
